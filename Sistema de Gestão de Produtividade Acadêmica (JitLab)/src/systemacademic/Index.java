...
public class Index {
	...
	static int line_projects = 100, columm_projects = 9 , line_collaborator = 100 , columm_collaborator = 9, line_prodAcademic = 100, columm_prodAcademic = 6, indexHistory = 0, indexPA = 0, line_history = 100, columm_history = 3;
	...
	static String [][] history = new String[line_history][columm_history];
			...
			if(login.equals("admin") && password.equals("123")) {
				...
				while(key_adm != 0) {
						...
						case 2:
							...
							if (projects[index_projectForParticipant][5] != null) {
								history[indexHistory][0] = collaborator[index_participants][0];
								history[indexHistory][1] = projects[index_projectForParticipant][0];
								// Separate date
								int i = Integer.parseInt(projects[index_projectForParticipant][5].replaceAll("/", ""));
								int d,m, y;
								y = i % 10000;
								int j = i / 10000;
								m = j % 100;
								...
								else mon = Integer.toString(m); 
								if(d > 0 && d < 10) {
									da = Integer.toString(d);
									da = "0" + da;
								}
								else da = Integer.toString(d);
								
								String aux = ye.concat(mon);
								String date = aux.concat(da);
														
								history[indexHistory][2] = date;
								indexHistory++;
							}
	...
	// Show sorted descending History Project by collaborator with Titles, Name of Conference and more
			public static void showHistoryProjectForCol(String name) {
				int year, yearz = 20301231, indexProj;
				while(true) {
					if(yearz == 20000101) break;
					for(int i = 0; i < 100; i++) {
						if(history[i][2] != null && history[i][0] != null) {
							year = Integer.parseInt(history[i][2]);
							if(year == yearz && history[i][0].equals(name) ) {
								if(history[i][1] != null) {
									System.out.println("Project: " + history[i][1]);
									indexProj = search_Projects(history[i][1]);
									if(indexProj >= 0) {
	...
	// Show sorted descending Production Academic by collaborator with Titles, Name of Conference and more
		public static void showProductionAcademicForCol(String title) {
			int year, yearz = 2030;
			while(true) {
				if(yearz == 2000) break;
				for(int i = 0; i < 100; i++) {
					if(prodAcademic[i][5] != null && prodAcademic[i][1] != null) {
						year = Integer.parseInt(prodAcademic[i][5]);
						if(year == yearz && prodAcademic[i][1].equals(title) ) {
							if(prodAcademic[i][0] != null)
							System.out.println("Project:" + prodAcademic[i][0]);
	...
	// Show sorted descending Production Academic by project with Titles, Name of Conference and more
	public static void showProductionAcademicForProj(String title) {
		int year, yearz = 2030;
		while(true) {
			if(yearz == 2000) break;
			for(int i = 0; i < 100; i++) {
	...
	public static void show_Collaborator(String name) {
		...
		if(indexCollaborator < 0) {
		    ...
		    System.out.println("");
		    showHistoryProjectForCol(name);
		    System.out.println("");
		    showProductionAcademicForCol(name);
	...
	public static void show_Project(String title) {
		...
		if(projects[indexProj][0] != null)
		    	...
		    	showProductionAcademicForProj(title);
	...
	public static void collaboratorProjectAssociated(int indexCollaborator) {
		...
		if(collaborator[indexCollaborator][3] == null) {
			...
			if(statusProject.equals("In preparation")) {
				...
				if (indexProject >= 0 && projects[indexProject][5] != null) {
					collaborator[indexCollaborator][3] = project;
					System.out.println("Sucess");
					
					history[indexHistory][0] = collaborator[indexCollaborator][0];
					history[indexHistory][1] = projects[indexProject][0];
					// Separate date
					int i = Integer.parseInt(projects[indexProject][5].replaceAll("/", ""));
					int d,m, y;
					y = i % 10000;
					int j = i / 10000;
					m = j % 100;
					...
					System.out.println("Sorry, complete the date of Project to add this collaborator");
				}
	...
	public static void edit_Project() {
			...
			while(op != 0) {
				...
				case 4:
					...
					break;
				case 5:
					if(projects[index_project][5] != null) {
					System.out.println("Initial Date of Project:" + "\nDay: ");
					day = input.nextInt();
					input.nextLine();
					...
					projects[index_project][5] = (Integer.toString(day) + "/" + Integer.toString(month) + "/" + Integer.toString(year) );
						...
						System.out.println("Sorry, but already set your date, you can't changed");
					}
					break;
				case 6:
